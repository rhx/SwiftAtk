...
public extension HyperlinkProtocol {
    ...
    @discardableResult @inlinable func onNotifyEndIndex(flags: ConnectFlags = ConnectFlags(0), handler: @escaping ( _ unownedSelf: HyperlinkRef, _ pspec: ParamSpecRef) -> Void ) -> Int {
        typealias SwiftHandler = GLib.ClosureHolder<(HyperlinkRef, ParamSpecRef), Void>
        let cCallback: @convention(c) (gpointer, gpointer, gpointer) -> Void = { unownedSelf, arg1, userData in
            let holder = Unmanaged<SwiftHandler>.fromOpaque(userData).takeUnretainedValue()
            let output: Void = holder.call((HyperlinkRef(raw: unownedSelf), ParamSpecRef(raw: arg1)))
            return output
        }
        return connect(
